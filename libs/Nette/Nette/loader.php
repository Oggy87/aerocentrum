...
new
ApplicationException('Too many loops detected in application life cycle.');}if(!$request){$this->onStartup($this);$router=$this->getRouter();Debug::addPanel(new
...
new
InvalidStateException("Cannot create journal file $this->file.");}}else{$writen=fwrite($init,pack('N2',self::FILEMAGIC,$this->lastNode));fclose($init);if($writen!==self::INT32SIZE*2){throw
new
...
new
InvalidStateException('Cannot acquite shared lock on journal.');}$header=stream_get_contents($this->handle,2*self::INT32SIZE,0);flock($this->handle,LOCK_UN);list(,$fileMagic,$this->lastNode)=unpack('N2',$header);if($fileMagic!==self::FILEMAGIC){fclose($this->handle);throw
new
...
new
InvalidStateException("Cannot load node number $id.");break;}$this->arrayAppendKeys($data,$childNode[self::INDEXDATA]);}return$data;}private
function
...
new
InvalidStateException("Cannot truncate journal file.");}}else{fseek($this->handle,self::HEADERSIZE+self::NODESIZE*$id);$writen=fwrite($this->handle,pack('N',0));if($writen!==self::INT32SIZE){throw
new
...
new
InvalidStateException('Cannot acquite exclusive lock on journal.');}if($this->lastModTime!==NULL){clearstatcache();if($this->lastModTime<@filemtime($this->file)){$this->nodeCache=$this->dataNodeFreeSpace=array();}}}}private
function
...
as$query){list($sql,$params,$time,$rows)=$query;$s.='<tr><td>'.sprintf('%0.3f',$time*1000).'</td><td class="database-sql">'.Connection::highlightSql(String::truncate($sql,self::$maxLength)).'</td><td>'.htmlSpecialChars(implode(', ',$params)).'</td><td>'.$rows.'</td></tr>';}return
...
as$key=>$value){if($key[0]==='['){unset($row[$key]);$key=substr($key,1,-1);$row[$key]=$value;}}return$row;}}class
DatabaseReflection
extends
